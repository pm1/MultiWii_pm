...
// **********************
  ...
  static uint32_t GPS_last_frame;
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if ( currentTime > calibratedAccTime ) {
  ...
  #if defined(GPS) && defined(GPS_LED_INDICATOR)       // modified by MIS to use STABLEPIN LED for number of sattelites indication
    ...
    if(currentTime > GPSLEDTime) {           // - Fix and sat no. >= 5 -> LED blinks, one blink for 5 sat, two blinks for 6 sat, three for 7 ...
      int8_t x = f.GPS_FIX ? GPS_numSat/2 :  ((millis() - GPS_last_frame) < 2000);
      if (x && (blcnt % 2)) {
        GPSLEDTime = currentTime + 100000;
        STABLEPIN_ON;
      }
      else {
        if (blcnt >= (2 * x)) {
          if (f.ANGLE_MODE || f.HORIZON_MODE) {STABLEPIN_ON;} else {STABLEPIN_OFF;}
          GPSLEDTime = currentTime + 2000000;
          blcnt = 0;
        }
        else {
          STABLEPIN_OFF;
